...
module RawgServices
  module RetryableGameDetailsRequest
    private

    def fetch_details_with_retry(game)
      attempts = 0
    ...
    def report_retry(game, attempts, retry_delay, error)
      ...
      @io.puts("Retry #{attempts}/#{max_retries} for #{game.slug} in #{retry_delay}s after #{error.message}")
    end

    def max_retries
      @max_retries.positive? ? @max_retries : ImportGameDetails::DEFAULT_MAX_RETRIES
    end
    ...
    def retryable_status_error?(error)
      ...
      status_code.present? && ImportGameDetails::RETRYABLE_STATUS_CODES.include?(status_code)
    end
